...
def run_provider_task(connection, project_paths, project_id, agent_id, task_id, provider_type, artifact_path=None):
    ...
    artifact_relative_path = artifact_path or ".maas/artifacts/{0}-{1}.txt".format(provider_type, task_id)
    artifact_full_path = (
        artifact_relative_path
        if os.path.isabs(artifact_relative_path)
        else os.path.join(project_paths.root, artifact_relative_path)
    )
